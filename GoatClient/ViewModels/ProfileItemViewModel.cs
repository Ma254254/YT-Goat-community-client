...
public sealed class ProfileItemViewModel : ObservableObject
{
    ...
    public bool IsInstalled => Services.Minecraft.LocalVersionStore.IsInstalled(Profile.MinecraftVersion);

    public string InstallStateText => IsInstalled ? "Installed" : "Not Installed";

    public string LastLaunchedText => Profile.LastLaunched is { } time
        ? $"Last played {time.ToLocalTime():yyyy-MM-dd HH:mm}"
        : "Never played";
