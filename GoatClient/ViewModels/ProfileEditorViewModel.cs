...
public sealed class ProfileEditorViewModel : ObservableObject
{
    ...
    public string JavaHint
    {
        get
        {
            var version = _versions.Find(MinecraftVersion);
            var major = _java.ResolveTargetMajor(JavaPreference, version);
            if (version is not null && major < version.RequiredJavaMajor)
            {
                return $"Minecraft {version.Id} requires Java {version.RequiredJavaMajor}. Java {major} will not work for this version.";
            }

            return $"Uses Java {major}, managed by GOAT CLIENT. No separate Java installation required.";
        }
    }
